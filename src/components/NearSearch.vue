    ...
    <div class="map-container">
      ...
      <div class="top-buttons">
        ...
        <div class="additional-buttons">
          <button
            class="hospital-button icon-circle"
          ...
          <button
            class="store-button icon-circle"
            @click="searchPlaces('편의점')"
          >
            <i class="fas fa-store"></i>
          </button>
          <button
            class="parking-button icon-circle"
            @click="searchPlaces('주차장')"
          >
...
const initMap = () => {
  ...
  const options = {
    center: new kakao.maps.LatLng(37.4987739, 126.97801),
    level: 5,
  };
...
const loadKakaoMap = () => {
  return new Promise((resolve) => {
    ...
    script.onload = () => {
      kakao.maps.load(() => {
        userMarkerImage.value = new kakao.maps.MarkerImage(
    ...
    script.src =
      "//dapi.kakao.com/v2/maps/sdk.js?autoload=false&appkey=b2a0131f4a53bec9033cfe18" +
      "aad009d5";
      ...
      (data, status) => {
        if (status === kakao.maps.services.Status.OK) {
          ...
          data.forEach((place) => {
            ...
            const marker = new kakao.maps.Marker({ map: map.value, position });
